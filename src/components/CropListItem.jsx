// src/components/CropListItem.jsx
import React from 'react';
import { Link } from 'react-router-dom'; // 1. Import Link

const CropListItem = ({ crop }) => {
  return (
    // 2. Wrap the list item with a Link component
    // The `to` prop constructs the URL, e.g., "/crop/Carrot"
    <Link to={`/crop/${crop.name}`}>
      <li className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm hover:bg-gray-50 cursor-pointer transition-colors">
